...
class BladeOne {
	...
	public function __construct( $templatePath = null, $compiledPath = null, $mode = 0 ) {
		...
		$this->templatePath = ( is_array( $templatePath ) ) ? $templatePath : array( $templatePath );
		...
		$this->authAnyCallBack = function ( $array = array() ) {
		...
		if ( $traits !== null ) {
			foreach ( $traits as $trait ) {
				$r    = explode( '\\', $trait );
				$name = end( $r );
				if ( is_callable( array( $this, $name ) ) && method_exists( $this, $name ) ) {
					$this->{$name}();
				}
			}
		}
	...
	protected function extendPush( $section, $content ) {
		if ( ! isset( $this->pushes[ $section ] ) ) {
			$this->pushes[ $section ] = array(); // start an empty section
		}
	...
	private function runInternal( $view, $variables = array(), $forced = false, $isParent = true, $runFast = false ) {
		...
		if ( @\count( $this->variablesGlobal ) > 0 ) {
			...
			$this->variablesGlobal = array(); // used so we delete it.
		} else {
